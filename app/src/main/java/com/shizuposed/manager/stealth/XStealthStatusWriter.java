 ...
 * side is never out of date. XStealth has no scope, so the file
 * contains only the master toggle and the per-check toggles.
 *
 * The file is written locally to external app storage first, then
 * copied to the shell side via the same sh -c 'cat ...' mechanism
 * that other module JSONs use.
 */
public final class XStealthStatusWriter {
     ...
     *
     * @param shellBase resolved shell base directory, from
     *                  ShizuPosedService.getResolvedShellBase()
     * @param shizuku   the helper to run the push through
     */
