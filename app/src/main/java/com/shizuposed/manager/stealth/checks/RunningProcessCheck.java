 ...
 *   • ActivityManager.getRunningTasks         → filters the result
 *
 * getRunningTasks is the fallback some apps use when the first two
 * return empty (either because they're restricted on the current
 * Android version, or because the framework suppresses them for
 * apps that don't hold REALTIME_TASK). Adding the filter makes the
 * hiding complete for apps that try the fallback.
 *
...
public final class RunningProcessCheck {
    ...
    public static void install(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        // getRunningTasks — deprecated on modern Android but still
        // callable via reflection and still used by some detectors.
        try {
            XposedHelpers.findAndHookMethod(
                amClass, "getRunningTasks", int.class,
                new XC_MethodHook() {
                    @Override protected void afterHookedMethod(MethodHookParam p) {
                        p.setResult(filterTasks(
                            (List<?>) p.getResult()));
                    }
                });
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": getRunningTasks hook failed: " + t);
        }
